...
func (c *Memory) Init(cfg config.Cfg, ollamaClient *api.Client, chat *chatui.ChatUI) error {
	c.cfg = cfg
...
func (c Memory) getEmbeddingsFromOpenAI(data string) []float32 {
	embeddings, err := c.ollamaClient.Embeddings(context.Background(), &api.EmbeddingRequest{
		Prompt: data,
	...
	embeddings32 := make([]float32, len(embeddings.Embedding))
	for i, v := range embeddings.Embedding {
