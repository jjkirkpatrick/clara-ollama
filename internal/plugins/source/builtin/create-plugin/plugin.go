...
func (c *CreatePlugin) Init(cfg config.Cfg, ollamaClient *api.Client, chat *chatui.ChatUI) error {
	c.cfg = cfg
	c.ollamaClient = ollamaClient
	c.chat = chat
	return nil
}
...
func (c *CreatePlugin) generatePluginCode(pluginDescription string) (string, error) {
	...
	response, err := c.sendRequestToOllama(c.conversation)
	...
	c.conversation = append(c.conversation, api.Message{
		Role:    "assistant",
...
func (c CreatePlugin) sendRequestToOllama(conversation []api.Message) (*api.ChatResponse, error) {
	...
	respFunc := func(resp api.ChatResponse) error {
		chatResponse = &resp
		c.cfg.AppLogger.Info(chatResponse)
		return nil
	}

	err := c.ollamaClient.Chat(ctx, req, respFunc)
	if err != nil {
...
import (
	"encoding/json"
	"errors"
	"github.com/jjkirkpatrick/clara/internal/chatui"
	"github.com/jjkirkpatrick/clara/internal/config"
	"github.com/jjkirkpatrick/clara/internal/plugins"
	"github.com/sashabaranov/go-openai"
	"github.com/sashabaranov/go-openai/jsonschema"
)
